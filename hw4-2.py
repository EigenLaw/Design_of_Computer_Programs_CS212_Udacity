...
def more_pour_problem(capacities, goal, start=None):
    ...
    def mp_successors(state):
        succ = {}
        unfilled = [c - state[i] for i, c in enumerate(capacities)]

        for i, filled in enumerate(state):
            # Empty the glass, if possible:
            if filled > 0:
                nstate = list(state)
                nstate[i] = 0
                succ[tuple(nstate)] = ('empty', i)

                # Pour into others, where possible:
                for i2, capacity in enumerate(capacities):
                    if i2 is not i and state[i2] < capacity:
                        if state[i] > unfilled[i2]:
                            poured = unfilled[i2]
                        else:
                            poured = state[i]
                        nstate = list(state)
                        nstate[i] = nstate[i] - poured
                        nstate[i2] = nstate[i2] + poured
                        succ[tuple(nstate)] = ('pour', i, i2)
            # Fill the glass, if possible:
            if filled < capacities[i]:
                nstate = list(state)
                nstate[i] = capacities[i]
                succ[tuple(nstate)] = ('fill', i)

        return succ

    mp_is_goal = lambda state: goal in state

    return shortest_path_search(start, mp_successors, mp_is_goal)
