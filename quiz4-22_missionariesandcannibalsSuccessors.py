...
#
# Created:     25-08-2013
# Copyright:   (c) santhosh 2013
# Licence:     <your licence>
#-------------------------------------------------------------------------------

from __future__ import division
from __future__ import print_function
"COMMENT THE ABOVE 2 LINES IF USING PYTHON 3.X"
...
def csuccessors(state):
    ...
    result = []
    for action, dm1, dc1, db1, dm2, dc2, db2 in (
        ('MM->', -2, 0, -1, 2, 0, 1),
        ('MC->', -1, -1, -1, 1, 1, 1),
        ('CC->', 0, -2, -1, 0, 2, 1),
        ('M->', -1, 0, -1, 1, 0, 1),
        ...
        ('<-CC', 0, 2, 1, 0, -2, -1),
        ('<-M', 1, 0, 1, -1, 0, -1),
        ('<-C', 0, 1, 1, 0, -1, -1)):    #iterate over all possiblities
        x = M1+dm1, C1+dc1, B1+db1, M2+dm2, C2+dc2, B2+db2
        if all(val>=0 for val in x):   #check if it is valid then append
            result.append((x, action))
    return dict(result)
